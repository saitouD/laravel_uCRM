...
const deleteItem = id => { //idについて、このidは下のdeleteConfirmの引数だけど、jsの場合関数に渡す引数が1つの場合、()を省略できるらしい
        Inertia.delete(route('items.destroy', { item: id}),{ 
            onBefore:() => confirm('本当に削除するの？')
        })
}
                                ...
                                <div class="lg:w-1/2 md:w-2/3 mx-auto">
                                    <div class="flex flex-wrap -m-2">
                                        <div class="p-2 w-full">
                                            <div class="relative">
                                                <label for="name" class="leading-7 text-sm text-gray-600">商品</label>
                                        ...
                                        <div class="p-2 w-full">
                                            <Link as="button" :href="route('items.edit', {item: item.id})" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">
                                                編集 <!--ここでLinkを使うために上でLinkをimport 通常のbuttonタグだとリクエストを飛ばすだけなので、Linkタグにした上でas button-->
                                                <!-- Link as button タグは :href で行先を決められる　 リンク Items/{item}　→　{item: item.id}　-->
                                            </Link><br>
